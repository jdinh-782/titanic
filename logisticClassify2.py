...
## METHODS ################################################################
    ...
    def plotBoundary(self,X,Y):
        ...
        # f(theta2 X2) == -theta0 - theta1 X1
        #              == (-theta0 - theta1 X1) / theta2 X2

        x2b_values = [(-self.theta[0]-(self.theta[1]*x1b[0]))/(self.theta[2]), (-self.theta[0]-(self.theta[1]*x1b[1]))/(self.theta[2])]
        # print(x2b_values)

        x2b = np.array(x2b_values);      # TODO find x2 values as a function of x1's values
        # print(x2b)
    ...
    def train(self, X, Y, initStep=1.0, stopTol=1e-4, stopEpochs=5000, plot=None):
        ...
        while not done:
            ...
            for i in np.random.permutation(M):
                ...
                inside_term = (1/(1 + np.exp(-ri)))

                if YY[i] == 1:
                    inside_term = -(1 - inside_term)

                gradi = inside_term * XX[i];     # TODO: compute gradient of NLL loss
            ...
            for i in np.random.permutation(M):
                ...
                if YY[i] == 1:
                    Jsur = Jsur + (-(YY[i] * np.log(inside_term)))
                else:
                    Jsur = Jsur + (-(1-(YY[i] * np.log(inside_term))))
    ...
    # train function for training with L1/L2 terms (Q1 Part 8/9)
    def train_with_regTerm(self, X, Y, initStep=1.0, stopTol=1e-4, stopEpochs=5000, alpha=0, reg_term="L1", plot=None):
        """ Train the logistic regression using stochastic gradient descent """
        M,N = X.shape;                     # initialize the model if necessary:
        self.classes = np.unique(Y);       # Y may have two classes, any values
        XX = np.hstack((np.ones((M,1)),X)) # XX is X, but with an extra column of ones
        ...
        while not done:
            ...
            for i in np.random.permutation(M):
                ...
                # now add L1 or L2 regularization term (by default is L1 term)
                # L1 -> + alpha * sum_i |theta_i|
                # L2 ->
                if reg_term:
                    gradi = gradi + (alpha * np.transpose(self.theta))
                else:
                    gradi = gradi + (2 * alpha * np.transpose(self.theta))
